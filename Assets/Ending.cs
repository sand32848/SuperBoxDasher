...
public class Ending : MonoBehaviour
{
    ...
    void Update()
    {
		...
		if (!inputController.LeftHold && isAiming)
		{
			...
			sequence.Append(player.transform.DOMove(new Vector3(2.05999994f, 0.5f, 0), 0.1f).From(new Vector3(-13.39f,-3,0)).SetEase(Ease.OutExpo).OnComplete(()=> {Instantiate(particle,new Vector3(0,0,0),Quaternion.identity); }));
			...
			sequence.Append(player.transform.DOMove(new Vector3(2.05999994f, 0.5f, 0), 0.2f).OnComplete(() => {congratUI.SetActive(true); }));
			...
			sequence1.Append(congratText1.GetComponent<RectTransform>().DOAnchorPos(new Vector3(-24, 169, 0), 2f).SetEase(Ease.OutQuad));
